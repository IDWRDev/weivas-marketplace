...
export const authOptions: NextAuthOptions = {
  ...
  providers: [CredentialsProvider({
    ...
    async authorize(credentials) {
      ...
      return { id: user.id, email: user.email, name: `${user.firstName} ${user.lastName}`, role: user.primaryRole, status: user.status, sellerApplicationStatus: user.sellerProfile?.applicationStatus };
    },
  })],
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        token.id = user.id; token.role = user.role; token.status = user.status; token.sellerApplicationStatus = user.sellerApplicationStatus;
      } else if (token.id) {
    ...
    session({ session, token }) {
      if (session.user) {
        session.user.id = token.id; session.user.role = token.role; session.user.status = token.status; session.user.sellerApplicationStatus = token.sellerApplicationStatus;
      }
      return session;
    },
  },
