...
export default async function AdminPage() {
  const [activeBuyers, verifiedSellers, liveProducts, protectedOrders, sellerReviews, productReviews, completedOrders, allOrders, openSupport] = await Promise.all([
  ...
  return <main className="admin-page"><header><div><small>WEIVAS OPERATIONS</small><h1>Marketplace control centre</h1><p>Live catalogue, seller and trust operations data.</p></div><div><Link className="button secondary" href="/admin/support">Support queue ({openSupport})</Link><Link className="button secondary" href="/">Return to marketplace</Link></div></header><section className="admin-metrics">{metrics.map(([Icon,label,value])=><Card key={label}><Icon/><small>{label}</small><strong>{value.toLocaleString()}</strong></Card>)}</section><section className="admin-grid"><Card><h2>Review queue</h2>{reviews.map(([label,value])=><div className="admin-row" key={label}><CircleAlert/><span><b>{label}</b><small>Requires operations review</small></span><strong>{value.toLocaleString()}</strong></div>)}</Card><Card className="admin-dark"><small>TRUST & SAFETY</small><h2>Marketplace health</h2><strong>{completionRate}</strong><p>{allOrders?"Orders completed without intervention":"Completion rate will appear after orders are completed."}</p></Card></section></main>;
}
