...
export function ProductCard({product,variant="standard"}:{product:Product;variant?:ProductVariant}){
  ...
  return <article className={cn("product-card",`product-${variant}`,product.status==="Out of Stock"&&"sold-out")}>
    ...
    <Link href={`/product/${product.slug}`}><div className="product-visual" role="img" aria-label={product.name}>{product.emoji}{product.status==="Out of Stock"&&<b>Sold out</b>}</div><div className="product-copy"><p>{product.name}</p><div><strong>${product.price.toFixed(2)}</strong>{product.oldPrice&&<del>${product.oldPrice.toFixed(2)}</del>}</div><small className="rating"><Star fill="currentColor"/> {product.rating} ({product.reviewCount?.toLocaleString()}) {product.sold?`· ${product.sold.toLocaleString()} sold`:""}</small>{seller&&variant!=="compact"&&<small className="seller-name">{seller.name}</small>}{product.verified&&<small className="verified-label"><BadgeCheck/> Verified seller</small>}{product.productVerified&&variant!=="compact"&&<small><PackageCheck/> Product verified</small>}{product.delivery&&<small>{product.delivery}</small>}{variant==="wholesale"&&<small className="moq">MOQ {product.moq} · {product.supplierCountry}</small>}</div></Link>
