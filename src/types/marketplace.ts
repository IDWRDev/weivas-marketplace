...
export interface Product{id:string;slug:string;name:string;description?:string;category:string;categorySlug:string;sellerId:string;sellerName?:string;sellerSlug?:string;price:number;oldPrice?:number;currency?:string;rating:number;reviewCount?:number;sold?:number;emoji:string;imageUrl?:string;imageAlt?:string;sku:string;stock:number;status:ProductStatus;verified?:boolean;productVerified?:boolean;delivery?:string;warranty?:string;returnPolicy?:string;supplierCountry?:string;moq?:number;brand?:string}
...
export interface CheckoutState{mode:CheckoutMode;items:CartItem[];checkoutKey:string}
